    ...
    buildAsync: function (sourceFiles) {
        ...
        sourceFiles.forEach(function(sourceFile) {
            try {
                ...
                console.log(("Parsing failed: " + e.toString()).red);
            }
        });

        var fileDependencyMaps = this.buildUsageMapSync(namespaces, sourceFiles);
        var finalResultJsSource = "$import._dependencyMaps = " + JSON.stringify(fileDependencyMaps) + ";";
                    ...
                    if(node.expression.type == "CallExpression" && node.expression.callee) {
                        ...
                        switch(callee.type) {
                            ...
                            case "MemberExpression":
                                ...
                                if(callee.object.name == "$namespace" && callee.property.name == "register") {
                        ...
                        case "CallExpression":
                            ...
                            if(callee.type == "MemberExpression") {
                                var condition = callee.object.type == "ThisExpression"
									|| node.expression.object.type == "Identifier" && node.expression.object.name == namespace.fullName;
                ...
                case "ExpressionStatement":
                    if(node.expression.type == "CallExpression" && node.expression.callee) {
                        ...
                        switch(callee.type) {
                            ...
                            case "MemberExpression":
                                // When registering a new namespace...
                                if(callee.object.name == "$namespace" && (callee.property.name == "using" || callee.property.name == "register")) {
                                    var scopeNamespaces = [];
                                ...
                                if(condition) {
                                    ...
                                    if(typeof foundMember != "undefined") {
                                        if(foundMember.hasOwnProperty("inherits")) {
                                            var files = that.getHierarchyDependenciesFromAstSync(foundMember, dependentFiles);
...
program
    .version("1.0.0")
    .option("-dir", "--directory", "Specifies the base directory from which the analyzer must start the search of JavaScript dependencies")
    .parse(process.hasOwnProperty("argsv") ? process.argsv : []);

if (!program.hasOwnProperty("directory") || program.directory == "") {
    console.warn("No base directory was given, using current directory...");

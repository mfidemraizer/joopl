...
DependencyAnalyzer.prototype = {
    start: function (args) {
        ...
        Object.defineProperty(
            ...
            "configuration", {
                value: {
                    ...
                    includes: args.hasOwnProperty("includes") ? args.includes : {},
                    fileExcludes: args.hasOwnProperty("fileExcludes") ? args.fileExcludes : [],
                    baseDirectoryOverrides: args.hasOwnProperty("baseDirectoryOverrides") ? args.baseDirectoryOverrides : {}
                },
                configurable: true
            }
        );
        ...
        this.loadFilesAsync(DependencyAnalyzer.configuration.baseDirectory).then(function (result) {
            var mapBuilder = new DependencyMapBuilder();
            that._map = mapBuilder.buildSync(result.sourceFiles, DependencyAnalyzer.configuration.baseDirectory, typeof result.includes == "undefined" ? {} : result.includes);
    ...
    loadFilesAsync: function (baseUri) {
        ...
        walker.on("end", function () {
            var allIncludePromises = [];

            if (DependencyAnalyzer.configuration.includes.length > 0) {
                DependencyAnalyzer.configuration.includes.forEach(function (includeUri) {
                    var includeContentsText;

                    if (includeUri.indexOf("http://") == 0 || includeUri.indexOf("https://") == 0) {
                        var defer = q.defer();
                        allIncludePromises.push(defer.promise);

                        http.get(includeUri, function (response) {
                            response.setEncoding('utf8');

                            var includeContentsText = "";

                            if (response.statusCode != 200) {
                                Logger.default.logWarning("Include with URI '%s' could not be parsed", includeUri);
                                defer.resolve({ includeUri: includeUri, includeContents: {} })
                            } else {
                                response.on("data", function (entityText) {
                                    includeContentsText += entityText;
                                });

                                response.on("end", function () {
                                    var text = includeContentsText.replace("$import.mapMany(", "").replace(");", "");
                                    defer.resolve({ includeUri: includeUri, includeContents: JSON.parse(text) });
                                });
                            }

                        });
                    } else {
                        includeContentsText = fs.readFileSync(includeUri, "utf8").replace("$import.mapMany(", "").replace(");", "");

                        addFiles(JSON.parse(includeContentsText));

                        deferred.resolve({ sourceFiles: sourceFiles });
                    }
                });
            }

            if (allIncludePromises.length > 0) {
                q.all(allIncludePromises).then(function (results) {
                    var includes = {};

                    results.forEach(function (result) {
                        includes[result.includeUri] = result.includeContents;
                    });

                    deferred.resolve({ sourceFiles: sourceFiles, includes: includes });
                });
            } else {
                deferred.resolve({ sourceFiles: sourceFiles });
            }
...
DependencyMapBuilder.prototype = {
    ...
    buildSync: function (sourceFiles, baseDirectory, includes) {
        var that = this;
        var namespaces = {};
        ...
        var fileDependencyMaps = this.buildUsageMapSync(namespaces, sourceFiles, includes);
    ...
    buildUsageMapSync: function (namespaces, sourceFiles, includes) {
        var that = this;
        var dependentFiles = {};

        Object.keys(includes).forEach(function (includeUri) {
            Object.keys(includes[includeUri]).forEach(function (moduleName) {
                if (!dependentFiles.hasOwnProperty(moduleName)) {
                    dependentFiles[moduleName] = includes[includeUri][moduleName];
                }
            });
        });
        ...
        Object.keys(this.parsedFiles).forEach(function (parsedFile) {
            try {
                ...
                that.getDependentFilesFromAstSync(parsedFile, ast, namespaces, dependentFiles[parsedFile], undefined, dependentFiles);
        ...
        var dependentFileKeys = Object.keys(dependentFiles);
        
        Object.keys(dependentFiles).forEach(function(fileName) {
            var memberInfo = underscore.filter(that.parsedMembers, function(item) { return item.sourceFile == fileName });
            
            if(typeof memberInfo != "undefined" && memberInfo.length > 0 && memberInfo[0].hasOwnProperty("inherits")) {
                var baseClassFile = dependentFiles[fileName][dependentFiles[fileName].indexOf(fileName) - 1];
                var indexOfCurrentFile = dependentFileKeys.indexOf(fileName);
                var indexOfBaseClassFile = dependentFileKeys.indexOf(baseClassFile);
                
                if(indexOfCurrentFile < indexOfBaseClassFile) {
                    dependentFileKeys.splice(indexOfBaseClassFile, 1);
                    dependentFileKeys.splice(indexOfCurrentFile - 1, 0, baseClassFile);
                }
            }
        });

        dependentFileKeys.forEach(function (uri) {
            try {
                ...
                if (path.extname(uri) == ".js") {
                    dependentFiles[uri].forEach(function (fileName) {
                        ...
                        if (that.parsedFiles.hasOwnProperty(fileName)) {
                            var ast = that.parsedFiles[fileName];

                            that.getDependentFilesFromAstSync(fileName, ast, namespaces, result, undefined, dependentFiles);
                                ...
                                if (files.indexOf(detectedFile) == -1) {
                                    ...
                                    if (typeof member != "undefined" && member.type == "class") {
                                        if (member.hasOwnProperty("inherits")) {
                                            ...
                                            if (that.parsedMembers.hasOwnProperty(baseClassFullName)) {
        ...
        dependentFileKeys.forEach(function (uri) {
            try {
                ...
                // NOTE: This is under investigation. It'll add these files that aren't dependencies of a class hierarchy...
                files.forEach(function (fileUri) {
                    var dependentFileKey = underscore.filter(Object.keys(dependentFiles), function (key) {
                        return underscore.last(dependentFiles[key]) == fileUri;
                    });

                    if (typeof dependentFileKey != "undefined" && dependentFiles.hasOwnProperty(dependentFileKey)) {
                        dependentFiles[dependentFileKey].forEach(function (file) {
        ...
        dependentFileKeys.forEach(function (fullFileName) {
            if (path.extname(fullFileName) == ".js") {
            ...
            } else if (!finalResult.hasOwnProperty(fullFileName)) {
                finalResult[fullFileName] = dependentFiles[fullFileName];
            }
        });
    ...
    getDependentFilesFromAstSync: function (parsedFile, ast, namespaces, dependentFiles, importedModules, allDependencies) {
        var that = this;

        ast.forEach(function (node) {
            switch (node.type) {
                case "FunctionExpression":
                    that.getDependentFilesFromAstSync(parsedFile, node.body.body, namespaces, dependentFiles, importedModules, allDependencies);
                    break;
                ...
                case "ExpressionStatement":
                    if (node.expression.type == "CallExpression" && node.expression.callee) {
                        ...
                        switch (callee.type) {
                            // When it's a self-invoked function
                            case "FunctionExpression":
                                that.getDependentFilesFromAstSync(parsedFile, callee.body.body, namespaces, dependentFiles, importedModules, allDependencies);
                                break;
                            ...
                            case "MemberExpression":
                                if (typeof callee.object != "undefined") {
                                    if (callee.object.name == "$import" && callee.property.name == "modules") {
                                        ...
                                        that.getDependentFilesFromAstSync(parsedFile, [callbackFunc], namespaces, dependentFiles, importedModules, allDependencies);
                                    }
                                        // When registering a new namespace...
                                    else if (callee.object.name == "$namespace" && callee.property.name == "using") {
                                        var namespacePaths = underscore.where(arguments, { type: "Literal" }).map(function (arg) {
                                        ...
                                        that.detectMembersFromNamespaceScopeSync(parsedFile, namespaceScopeAst, namespaceAliases, namespaces, dependentFiles, importedModules, allDependencies);
                                    }
                                }
                                break;
                        }
    ...
    detectMembersFromNamespaceScopeSync: function (parsedFile, namespaceScopeAst, namespaceAliases, namespaces, dependentFiles, importedModules, allDependencies) {
        var that = this;
                                ...
                                if (typeof ns != "undefined") {
                                    ...
                                    if (typeof foundMember != "undefined") {
                                        if (foundMember.hasOwnProperty("inherits")) {
                                            var files = that.getHierarchyDependenciesFromAstSync(foundMember, dependentFiles, allDependencies);
                                        ...
                                        if (hasMembers && declarationOptions.members.type == "ObjectExpression") {
                                            ...
                                            classMembers.forEach(function (memberAst) {
                                                if (memberAst.value.type == "FunctionExpression") {
                                                    ...
                                                    var newInstances = memberContent.match(that.newInstanceRegex);
                                                    ...
                                                    if (result != null) {
                                                        ...
                                                        var tempMemberName;
                                                        var tempMemberNameParts;

                                                        result.forEach(function (newExpressionText) {
                                                            switch (newExpressionText.indexOf("$global")) {
                                                                case -1:
                                                                    tempMemberName = newExpressionText.replace("new", "").replace(" ", "");
                                                                    tempMemberNameParts = tempMemberName.split(".");
                                                                    tempMemberName = namespaceAliases[tempMemberNameParts[0]] + "." + tempMemberNameParts[1];

                                                                    if (namespaceAliases.hasOwnProperty(tempMemberNameParts[0])) {
                                                                        innerFoundMember = underscore.findWhere(namespaces[namespaceAliases[tempMemberNameParts[0]]].members, { name: tempMemberNameParts[1] });
                                                                    }
                                                            ...
                                                            if (typeof innerFoundMember != "undefined") {
                                                                if (that.parsedFiles.hasOwnProperty(innerFoundMember.sourceFile)) {
                                                                    var hierarchyModules = that.getHierarchyDependenciesFromAstSync(innerFoundMember, dependentFiles, allDependencies);
                                                            ...
                                                            } else if (allDependencies.hasOwnProperty(tempMemberName)) {
                                                                var files = allDependencies[tempMemberName];
                                                                var includedFiles = [];

                                                                for (var fileIndex in files) {
                                                                    if (memberDependentFiles.indexOf(files[fileIndex]) == -1) {
                                                                        includedFiles.push(files[fileIndex]);
                                                                    }
                                                                }

                                                                includedFiles.splice(0, 0, 0, 0);

                                                                memberDependentFiles.splice.apply(memberDependentFiles, includedFiles);
                                                            }
    ...
    getHierarchyDependenciesFromAstSync: function (memberAst, dependentFiles, allDependencies) {
        ...
        if (typeof baseAst != "undefined") {
            do {
                ...
                if (this.parsedMembers.hasOwnProperty(tempFullName)) {
                    ...
                    foundFiles.push(tempParsedMember.sourceFile);

                    baseAst = tempParsedMember.hasOwnProperty("inherits") ? tempParsedMember.inherits : undefined;
                } else if (allDependencies.hasOwnProperty(tempFullName)) {
                    var files = allDependencies[tempFullName];
            ...
            foundFiles.splice.apply(foundFiles, includedFiles);
        }
